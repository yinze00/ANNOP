...
def tf_gen_op_libs(op_lib_names, deps = None, is_external = True):
    ...
    for n in op_lib_names:
        native.cc_library(
            ...
            # copts = 
            srcs = ["ops/" + n + ".cc"],
            deps = deps + [clean_dep("@tensorflow//:tensorflow_framework")],
...
def tf_gen_op_wrapper_cc(
    ...
    native.genrule(
        ...
        srcs = srcs,
        tools = [":" + tool] + [clean_dep("@tensorflow//:tensorflow_framework")],
...
def tf_gen_op_wrappers_cc(
    ...
    native.cc_library(
        ...
        srcs = subsrcs,
        hdrs = subhdrs,
        deps = deps + ["@tensorflow//:tensorflow_framework"],
        # copts = tf_copts(),
        alwayslink = 1,
    ...
    native.cc_library(
        ...
        srcs = internalsrcs,
        hdrs = internalhdrs,
        deps = deps + deps_internal + ["@tensorflow//:tensorflow_framework"],
        # copts = tf_copts(),
        alwayslink = 1,
        visibility = visibility,
    )
